...
import { clearAppLocalCache } from '@/lib/local-cache';
...
type HeaderProps = {
  ...
  showClearStorageButton?: boolean;
};
...
export default function Header({
  ...
  showClearStorageButton = false,
}: HeaderProps) {
  const pathname = usePathname();
  const { isDarkMode, toggleTheme } = useTheme();
  const isHomeActive = showRouteMenu && (pathname === '/' || pathname === '/dashboard');
      ...
      <div className={styles.headerInner}>
        <div className={styles.headerControls}>
          {showRouteMenu ? (
            ...
            <div className={styles.headerFiller} aria-hidden="true" />
          )}
